const data = [
  { id: 1, color: "linear-gradient(to right, #0f2027, #203a43, #2c5364)" },
  { id: 2, color: "linear-gradient(90deg, #C79A1F 0%, #2A145A 100%)" },
  { id: 3, color: "linear-gradient(90deg, #7A9110 0%, #00673F 100%)" },
  { id: 4, color: "linear-gradient(90deg, #D32F58 0%, #2F4ACC 100%)" },
  { id: 5, color: "linear-gradient(90deg, #0099CC 0%, #2D39B0 100%)" },
  { id: 6, color: "linear-gradient(90deg, #1691C6 0%, #000539 100%)" },
  { id: 7, color: "linear-gradient(to left, #B11E0D, #C78E16)" },
  { id: 8, color: "linear-gradient(to right, #6A22B5, #3700B0)" },
];

export default data;
